...
def main() -> None:
    ...
    ap.add_argument("--train_csv", type=str, default="data/pairs/train.csv")
    ap.add_argument("--val_csv", type=str, default="data/pairs/val.csv")
    ...
    ap.add_argument(
        "--sample_nocrop",
        action="store_true",
        help="If set, samples are generated from a no-crop visualization dataset (resize directly to image_size).",
    )
    ...
    ds_train = PairedChessDataset(
        ...
        require_masks=(not args.no_masks),
    )
    ...
    viz_batch = None
    if args.sample_nocrop:
        # Use VAL csv (more representative) but with NO crop: load_size == image_size
        ds_viz = PairedChessDataset(
            args.val_csv,
            repo_root=".",
            image_size=args.image_size,
            load_size=args.image_size,   # critical: no extra resize then crop
            train=False,
            hflip_prob=0.0,
            seed=args.seed,
        )
        dl_viz = DataLoader(ds_viz, batch_size=1, shuffle=False, num_workers=0)
        viz_batch = next(iter(dl_viz))
    ...
    while step < args.max_steps:
        ...
        with autocast():
            ...
            g_piece = torch.tensor(0.0, device=device)
            if (not args.no_masks) and (M is not None) and (args.lambda_piece > 0):
                diff = torch.abs(fake_B - B)      # (N,3,H,W)
                mask3 = M.expand_as(diff)         # (N,3,H,W), M is (N,1,H,W)
                g_piece = (diff * mask3).sum() / (mask3.sum() + 1e-6)
                g_piece = g_piece * args.lambda_piece

            g_loss = g_gan + g_l1 + g_grad + g_piece
        ...
        if args.sample_every > 0 and step % args.sample_every == 0:
            ...
            use_batch = viz_batch if (args.sample_nocrop and viz_batch is not None) else batch
            save_samples(G, use_batch, device, out)
