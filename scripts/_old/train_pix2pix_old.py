...
# Checkpointing
# ----------------------------
def save_checkpoint(path: Path, step: int, best_val: float, G, D, optG, optD, D_piece=None) -> None:
    payload = {
        "step": step,
        "best_val": best_val,
        "G": G.state_dict(),
    ...
    if D_piece is not None:
        payload["D_piece"] = D_piece.state_dict()
    torch.save(payload, path)


def load_checkpoint(path: Path, G, D, optG, optD, strict: bool = True, D_piece=None) -> tuple[int, float]:
    ...
    d_state = ckpt.get("D", {})
    d_loaded = False
    run_has_piece = D_piece is not None
    try:
    ...
    ckpt_has_piece = ("D_piece" in ckpt)
    piece_loaded = (not run_has_piece)
    if run_has_piece:
        if ckpt_has_piece:
            try:
                D_piece.load_state_dict(ckpt["D_piece"], strict=strict)
                piece_loaded = True
                print("[INFO] Loaded D_piece from checkpoint")
            except Exception as e:
                print(f"[WARN] Could not load D_piece weights: {e}")
                print("[INFO] Using randomly initialized D_piece")
                piece_loaded = False
        else:
            print("[WARN] Checkpoint has no D_piece, but current run uses --use_piece_D.")
            print("[INFO] D_piece will be randomly initialized and optD will be reset (no optD resume).")
            piece_loaded = False
    ...
    # Load discriminator optimizer ONLY if the discriminator structure matches (piece/no-piece)
    can_resume_optD = d_loaded and ((ckpt_has_piece == run_has_piece) and (piece_loaded or (not run_has_piece)))
    if can_resume_optD:
        try:
        ...
        print("[INFO] Using fresh discriminator optimizer (structure mismatch or missing D_piece)")
...
def main() -> None:
    ...
    ap.add_argument("--canonical_size", type=int, default=None,
                    help="If set, first canonicalize images to this size (e.g., 480) before optional padding.")
    ap.add_argument("--pad_to", type=int, default=None,
                    help="If set, pad canonical images to this size (e.g., 512) using reflect (images) / zero (mask).")
    ap.add_argument("--synth_crop_border", type=int, default=16,
                    help="When canonical_size=480 and synth is 512x512, shave this many px per side (default: 16).")
    ...
    if resume_path is not None:
        ...
        step, best_val = load_checkpoint(resume_path, G, D, optG, optD, strict=False, D_piece=D_piece)
        print(f"[INFO] resumed from {resume_path} (step={step}, best_val={best_val:.4f})")

        # Checkpoints restore optimizer hyperparams (LR/betas). Override them to match CLI flags.
        for pg in optG.param_groups:
            pg["lr"] = args.lr
            pg["betas"] = (args.beta1, args.beta2)
        for pg in optD.param_groups:
            pg["lr"] = args.lr
            pg["betas"] = (args.beta1, args.beta2)
        print(f"[INFO] override optimizer hyperparams after resume: lr={args.lr} betas=({args.beta1},{args.beta2})")
    ...
    while step < args.max_steps:
        ...
        if args.val_every > 0 and step % args.val_every == 0:
            ...
            # Always save latest
            save_checkpoint(ckpt_latest, step, best_val, G, D, optG, optD, D_piece=D_piece)

            # Save best by val L1
            if mean_l1 < best_val:
                best_val = mean_l1
                save_checkpoint(ckpt_best, step, best_val, G, D, optG, optD, D_piece=D_piece)
            ...
            if args.use_piece_mask and ('piece_sharpness' in metrics):
                ...
                if delta > best_piece_delta:
                    ...
                    save_checkpoint(ckpt_best_piece, step, best_val, G, D, optG, optD, D_piece=D_piece)
